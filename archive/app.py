...
# --- Page Config ---
st.set_page_config(
    page_title="Convenient Meetup Finder", 
    layout="wide",
    page_icon="📍"
)
...
def check_password():
    ...
    # 1. Check if password exists in Secrets
    if "app_password" not in st.secrets:
        # If no password is in the vault, we just let people in (or you could stop them)
        return True
    ...
    def password_entered():
        ...
        if st.session_state["password"] == st.secrets["app_password"]:
            ...
            del st.session_state["password"]  # clear the input field for security
        else:
    ...
    if "password_correct" not in st.session_state:
        ...
        # Password incorrect, show input again with error
        st.text_input(
            "Please enter the app password:", 
            type="password", 
            on_change=password_entered, 
...
with st.sidebar:
    ...
    # --- API KEY LOGIC (Use Secret if available) ---
    if "gmaps_api_key" in st.secrets:
        api_key = st.secrets["gmaps_api_key"]
        # We don't show an input field here because we found the secret!
    else:
...
if submit_button:
    if not api_key:
        st.error("Google Maps API Key not found. Please check your secrets.toml file.")
...
if not submit_button:
    st.info("Enter locations to start searching.")
